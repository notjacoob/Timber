    ...
    run = (inter:CommandInteraction,opts:any)=>{
        if (opts.player._playing) {
            const embed = new MessageEmbed()
                ...
                .addFields(
                    { name: `Title`, value: `${opts.player._current?.track.title}`, inline: false },
                    { name: 'Queued by', value: `${opts.player._current?.by.user.username}`, inline: true },
                    { name: 'Channel', value: parseLength(opts.player._current?.track.channel.name), inline: true },
                    { name: 'Length', value: new Date(Number(opts.player._current?.track.durationInSec) * 1000).toISOString().substr(11, 8), inline: true },
                    { name: `Link`, value: `${opts.player._current?.track.url}`, inline: false }
                )
